...
function runSearchRuby(options, timeout) {
    ...
    function checkSearchRuby() {
        ...
        if ((SO.curr > SO.total) || ((elapsed + last_el) > timeout)) {
            ...
            if (SO.curr <= 1) {
                ...
                Ruby.call('Printing','rho_searchPrinters?options='+options);
...
$(window).load(
    function() {
        var conn_types = [Rho.Printer.CONNECTION_TYPE_BLUETOOTH, Rho.Printer.CONNECTION_TYPE_ON_BOARD, Rho.Printer.CONNECTION_TYPE_TCP, Rho.PrinterZebra.CONNECTION_TYPE_USB];
